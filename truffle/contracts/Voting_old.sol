...
contract VotingOld is Ownable {
    ...
    uint private totalVotesCount;
    mapping (address => Voter) private voters;
    Proposal[] private proposals;
    uint private MAXPROPOSALS_NB; /* To counter Gas Limit DoS */
    ...
    **/
    function registerProposal(string calldata _description) external onlyStatus(WorkflowStatus.ProposalsRegistrationStarted) onlyRegisteredVoters {
        ...
        require (_id < MAXPROPOSALS_NB, "Maximum nb of proposals reached");  /* To counter Gas Limit DoS */
    ...
    **/
    function vote(uint _id) external onlyStatus(WorkflowStatus.VotingSessionStarted) onlyRegisteredVoters {
     ...
     * @param _id the id of the proposal to vote for.
    **/
    function bribe(address _addr, uint _id) external payable onlyStatus(WorkflowStatus.VotingSessionStarted) onlyRegisteredVoters {
    ...
    **/
    function _tallyHandleTie() internal {
        uint _maxCount;
        uint _tmpTotalVotesCount;

        // Finds the Proposal(s) with the greatest vote count, and stores them and their _ids in winningProposals and winningProposalIds
        for (uint _id=0; _id<proposals.length; _id++) {
            if (proposals[_id].voteCount >= _maxCount) {
                ...
                if (proposals[_id].voteCount > _maxCount) {
                    while (winningProposalIds.length != 0)
                        winningProposalIds.pop();
                    while (winningProposals.length != 0)
                        winningProposals.pop();
                }
                winningProposalIds.push(_id);
                winningProposals.push(proposals[_id]);
                _maxCount = proposals[_id].voteCount;
            }
            _tmpTotalVotesCount += proposals[_id].voteCount;
        }
        totalVotesCount = _tmpTotalVotesCount;
        ...
        if (winningProposalIds.length == 1) { 
            ...
            while (proposals.length != 0)
                proposals.pop();
            uint _winningProposalsNb = winningProposals.length;
            while (proposals.length != _winningProposalsNb) {
                proposals.push(winningProposals[winningProposals.length - 1]);
    ...
    **/
    function getWinner() external view onlyStatus(WorkflowStatus.VotesTallied) returns(uint) {
